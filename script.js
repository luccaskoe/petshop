import { salvarMembroClub } from "./club.js";
import { salvarCliente } from "../adm-petshop-main/firebase-clientes.js";
...
const btnAssinarPlano = document.getElementById("btnAssinarPlano");

const raInicio = document.getElementById("raInicio");
const raVencimento = document.getElementById("raVencimento");

raInicio?.addEventListener("change", () => {

  if (!raInicio.value) {
    raVencimento.textContent = "--";
    return;
  }

  const inicio = new Date(raInicio.value);
  const vencimento = new Date(inicio);

  vencimento.setMonth(vencimento.getMonth() + 1);

  raVencimento.textContent = vencimento.toLocaleDateString("pt-BR");
...
if (btnRAAssinar) {
  ...
  btnRAAssinar.addEventListener('click', (e) => {

    // Impede que o <a href=""> tente abrir outra página
    e.preventDefault();

    agendamentoContexto.raclub = {
      status: 'assinando'
    };

    salvarContextoSessao();

    // Esconde os três botões
    document.querySelector(".row-actions").style.display = "none";

    // Mostra o formulário
    document.getElementById("raForm").classList.remove("hidden");

    document.getElementById("raForm").setAttribute("aria-hidden", "false");

  });

}

// ===== Regras de Asiinantes =====

btnAssinarPlano?.addEventListener("click", async () => {
  console.log("BOTÃO CLICADO");

  const plano = document.querySelector('input[name="plano"]:checked')?.value;
  const inicio = document.getElementById("raInicio").value;
  const vencimento = document.getElementById("raVencimento").textContent.trim();
  const pet = document.getElementById("raPet").value.trim();
  const especie = document.getElementById("raEspecie").value;
  const raca = document.getElementById("raRaca").value.trim();
  const idade = document.getElementById("raIdade").value;

  if (!plano) {
    Swal.fire({ icon: "warning", title: "Escolha um plano" });
    return;
  }

  if (!inicio) {
    Swal.fire({ icon: "warning", title: "Selecione a data de início" });
    return;
  }

  if (!pet) {
    Swal.fire({ icon: "warning", title: "Informe o nome do pet" });
    return;
  }
  ...
  await salvarMembroClub(db, {
    nome: agendamentoContexto.nomeCliente,
    sobrenome: agendamentoContexto.sobrenomeCliente,
...
async function preencherHorasDisponiveis() {
  ...
  try {
    ...
    console.error("Erro ao carregar horários:", e);
     Swal.fire({ icon: "error", title: "Não foi possível consultar os horários agora." })
  }
}
...
const firebaseConfig = {
  apiKey: "AIzaSyBSafLT7SuFSBR2pDeEgC8hP0LlXpBlSK8",
  authDomain: "pet-shop-test-908d8.firebaseapp.com",
  projectId: "pet-shop-test-908d8",
  storageBucket: "pet-shop-test-908d8.firebasestorage.app",
  messagingSenderId: "856362564765",
  appId: "1:856362564765:web:b28610c71997fed822a874"
...
dataInput?.addEventListener('input', async () => {
  ...
  try {
    ...
    if (disp?.fechado === true) {
      Swal.fire({ icon: "warning", title: "Indisponível nesta data. Escolha outro dia." });
      dataInput.value = "";
      horaSelect.innerHTML = `<option value="">Selecione a data e depois o serviço</option>`;
      horaSelect.value = "";
      horaSelect.disabled = true;
...
confirmarBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    const telefoneSnap = await getDocs(
      query(
        collection(db, col),
        where("clienteTelefone", "==", agendamentoContexto.telefoneCliente),
        where("data", "==", data)
      )
    );

    if (!telefoneSnap.empty) {
      alert("Este telefone já possui um agendamento nesta data.");
      confirmarBtn.disabled = false;
      confirmarBtn.textContent = originalText;
      return;
    }

    await salvarCliente(db, {
